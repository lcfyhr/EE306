...
step1	LDR R2, =SSI0_SR_R
		LDR R1, [R2]
		AND R3, R1, #0x10				; step 1 and 2
		CMP R3, #0
		BNE step1
		
		LDR R2, =DC
		LDR R1, [R2]
		BIC R1, R1, #0x40				; step 3
		STR R1, [R2]
		
		LDR R2, =SSI0_DR_R				; step 4
		STRB R0, [R2]
		
step5	LDR R2, =SSI0_SR_R
		LDR R1, [R2]
		AND R1, R1, #0x10				; step 5 and 6
		CMP R1, #0
		BNE step5
		BX  LR                          ;   return
...
step11	LDR R2, =SSI0_SR_R
		LDR R1, [R2]
		AND R1, R1, #0x02				; step 1 and 2
		CMP R1, #0
		BEQ step11
		
		LDR R2, =DC
		LDR R1, [R2]
		ORR R1, R1, #0x40				; step 3
		STR R1, [R2]
